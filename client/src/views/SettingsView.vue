      ...
      <div class="space-y-4">
        <div class="bg-white dark:bg-gray-800 rounded-xl p-4">
          ...
          <div class="flex gap-2">
            <button v-for="size in ['sm', 'md', 'lg']" :key="size" @click="fontSize = size" :class="fontSize === size ? 'bg-primary text-white' : 'bg-gray-200 dark:bg-gray-700'" class="px-4 py-2 rounded-lg">
              {{ size === 'sm' ? '小' : size === 'md' ? '中' : '大' }}
            </button>
          </div>
        </div>
        <div class="bg-white dark:bg-gray-800 rounded-xl p-4">
          <h3 class="font-medium mb-3">主题</h3>
          <p class="text-sm text-gray-500">跟随系统自动切换明暗主题</p>
        </div>
        <button @click="handleLogout" class="w-full bg-gray-200 dark:bg-gray-700 py-3 rounded-xl text-gray-700 dark:text-gray-300">退出登录</button>
      </div>
    </main>
